...
def main():
    ...
    total_tokens = os.environ.get("TOTAL_TOKENS", "0")
    total_cost_usd = os.environ.get("TOTAL_COST_USD", "")
    ...
    # Build usage stats footer
    usage_parts = []
    try:
        total_tok = int(total_tokens)
        if total_tok > 0:
            usage_parts.append(f"{total_tok:,} tokens")
            # Add breakdown if available
            inp_tok = int(input_tokens)
    ...
    usage_footer = ""
    if usage_parts:
        usage_footer = f"\n<sub>Usage: {' · '.join(usage_parts)}</sub>"
    ...
    comment_body = (
        ...
        f"dependency security review</sub>{usage_footer}"
    )
